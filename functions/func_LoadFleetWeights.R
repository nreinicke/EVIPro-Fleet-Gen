...
#       Must have 14 columns of data, 2 columns for each of seven fleet characteristics in the following order from left to right
#           pev_type, preferred_loc, power_home, power_work, power_public, temp_c, schedule_vmt_bin
...
#                   The sum of all weights must equal 1 for each option.
...
load_fleet_weights <- function(weights_file) {
  ...
  temp_weights <- as.data.table(fleet_weights[11:12])
  vmt_weights <- as.data.table(fleet_weights[13:14])
