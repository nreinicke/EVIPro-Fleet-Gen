...
Unregister function to 'clean' up the futures 
This is an important step in the for loop as otherwise you will be haunted by ghosts of 'futures' past, which may cause catastrophic memory issues. 
...
getDoParName() # get parallel backend name
getDoParWorkers() # get parallel backend workers (cores)

registerDoSEQ() # register default sequential / backup to unregister function
 #lapply(temp_vec, function(temp) {
for(i in 1:length(temp_vec)) {

	# register the future each iteration of the for loop
	# Using the DoParallel() instead of DoFuture() to try to stay within the foreach library of functions
  ...
  evi_raw <- readRDS(paste0("../../data/preprocessed/evi_raw/", temp_vec[i], ".rds"))
  ...
  # evi_raw_suv <- readRDS(evi_data_path_suv)
		evi_raw_suv <- readRDS(paste0("../../data/preprocessed/evi_raw_suv/", temp_vec[i], ".rds"))
  # add column for vehicle class
  ...
  evi_load_profiles <- readRDS(paste0("../../data/preprocessed/load_profile/", temp_vec[i], ".rds"))
  ...
  evi_load_profiles_suv <- readRDS(paste0("../../data/preprocessed/load_profile/", temp_vec[i], ".rds"))
  # add vehicle class
      ...
      foreach(numveh = fleet_size_vec, .combine='rbind') %do% {
        sub_fleet_load <-
          ...
          foreach(loc = loc_class_vec, .combine='rbind') %:%
            ...
            # add check for distribution of suvs and sedans
            # For some reason warnings aren't being displayed with this type of parallel processing
            # Testing within the functions show the ratio of suv/sedan to be preserved
         #   warning(print(load_to_bind[, .(perc = .N / nrow(load_to_bind) * 100), by = vehicle_class]))
...
fleet_load[, mean_dvmt := as.integer(mean_dvmt)]
  ...
  # The memory should be roughly the same size each time
  warning(print(object.size(x=lapply(ls(), get)), units="Mb"))
} # end of for loop
#})
Sys.time()
```

Rewriting as a futures parallel
...
lapply(temp_vec, function(temp) {
  ...
  fleet_load <- future_lapply(all_options_list, function(options_list) {
  	...
  	load_to_bind <- openEVI(evi_raw = evi_raw,
  									evi_load_profiles = evi_load_profiles,
  									fleet = options_list$numveh,
  									pev = options_list$pev,
  									dvmt = options_list$dvmt,
  									pref = options_list$pref,
  									home = options_list$home,
  									work = options_list$work,
  									loc = options_list$loc,
  									vclass - options_list$vclass)

  	
  	## Need to determine how to name the permutations
  	# probably a lookup table replace of some sort .. 

  	# Create Names ----------------------------------------------------------------------- 
		 
  	# Match the options list with the named vector
		 # PEV DIST
  	 pev <- lapply(pev_type_vec, function(x) {
		 	x[x %in% unlist(options_list$pev)]
		 	})
			# filter out length zero lists
		 pev <- names(Filter(length, pev))
  	
		 # Charge Preference
		 pref <- lapply(home_power_vec, function(x) {
		 	x[x == options_list$home[[1]]]
		 })
		 pref <- names(Filter(length, pref))
		 
		 
		 getNames(pref_vec, options_list$pref)
...
load_to_bind <- data.table(t = rnorm(100, 1, 1),
																											v = rnorm(100, 1, 1))
            load_to_bind[, ':=' (loc_class = options_list$loc[[1]],
            																					...
            																					perf_dist = getNames(pref_vec, options_list$pref),
            																					home_access = strsplit(getNames(home_power_vec, options_list$home), "_")[[1]][1],
            																					home_power_dist = strsplit(getNames(home_power_vec, options_list$home), "_")[[1]][2]
            													)]
...
home <-
 getNames(home_power_vec, options_list$home)
