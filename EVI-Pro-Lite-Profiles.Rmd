...
for(i in 1:length(temp_vec)) {
  ...
  # load the raw data for the
  evi_data_path <- paste0("../../data/preprocessed/evi_raw/", temp_vec[i], ".rds")
  evi_raw <- readRDS(evi_data_path)
  
  # load the load profiles
  loadprof_data_path <- paste0("../../data/preprocessed/load_profile/", temp_vec[i], ".rds")
  evi_load_profiles <- readRDS(loadprof_data_path)
  ...
  fleet_load <-
      foreach(numveh = fleet_size_vec, .combine='rbind') %do% {
        sub_fleet_load <-
          ...
          foreach(loc = loc_class_vec, .combine='rbind') %dopar% {
            # TODO add SUV vehicle class here
            ...
            load_to_bind <- load_to_bind[, .(kw = sum(avg_kw)),
                                         by = c("loc_class",
                                                ...
                                                "dest_chg_level",
                                                "time_of_day")]
            setkey(load_to_bind,loc_class,temp_c,fleet_size,mean_dvmt,pev_dist,pref_dist,home_access,home_power_dist,work_power_dist,day_of_week,pev_type,dest_type,dest_chg_level,time_of_day)
  ...
  fwrite(fleet_load,
  										file = paste0("output/EVI-Pro-Lite_Run/",
